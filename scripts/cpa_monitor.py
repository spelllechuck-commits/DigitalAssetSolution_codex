...
EVENT_BASE={'ETF':3.0,'ADOPTION':2.0,'REGULATION':2.5,'HACK':4.0,'DEPEG':5.0,'EXCHANGE':3.5,'MACRO':2.5,'PRICE':1.5,'OTHER':1.0}
SEV={'LOW':.5,'MEDIUM':1.0,'HIGH':1.5,'CRITICAL':2.0}
...
def universe():
    ...
    with ThreadPoolExecutor(max_workers=2) as ex:
        a=ex.submit(cg,'/coins/markets',{**p,'per_page':250,'page':1}).result() or [];b=ex.submit(cg,'/coins/markets',{**p,'per_page':50,'page':6}).result() or []
...
def tech(c,cache):
    ...
    out={'ok':False,'source':'coingecko-live'};d=cg(f"/coins/{c['id']}/market_chart",{'vs_currency':'usd','days':'365','interval':'daily'});time.sleep(1.2);h=cg(f"/coins/{c['id']}/market_chart",{'vs_currency':'usd','days':'30'});time.sleep(1.2)
    try:
        daily=[float(x[1]) for x in d['prices']];four=[float(x[1]) for x in h['prices']][::4]
        if len(daily)>=200 and len(four)>20:out.update(ok=True,e20=ema(daily,20),e50=ema(daily,50),e200=ema(daily,200),rsi=rsi(four),ts=now)
...
def deriv(c):
    ...
    return out
def enrich(c,cache):
    t=tech(c,cache);d=deriv(c);t.update(d);t['source']=t.get('source','coingecko')+('+binance' if d.get('deriv') else '+no-derivatives');return t
def active_events():
    feed=load_json(EVENT_PATH,{'market_risk':'LOW','events':[]});now=datetime.now(timezone.utc);seen=set();a=[]
...
def events_for(symbol,events):return [e for e in events if e.get('scope')=='MARKET' or symbol in event_symbols(e)]
def decay(e):
    try:age=(datetime.now(timezone.utc)-datetime.fromisoformat(e['timestamp'].replace('Z','+00:00'))).total_seconds()/86400
    except Exception:return .4
    return 1.0 if age<=1 else .7 if age<=3 else .4 if age<=7 else .15
def event_value(e):
    sign=1 if e.get('direction')=='POSITIVE' else -1 if e.get('direction')=='NEGATIVE' else 0
    return sign*EVENT_BASE.get(e.get('type'),1.0)*SEV.get(e.get('severity'),1.0)*decay(e)
def event_overlay(status,symbol,events,market_risk):
    es=events_for(symbol,events);vals=[event_value(e) for e in es];net=sum(vals);base=status;highneg=any(event_value(e)<=-4 and e.get('confidence')=='HIGH' for e in es)
    if (highneg or market_risk in ('HIGH','CRITICAL')) and status in ('READY','NEAR ENTRY'):status='EVENT HOLD'
    risk='HIGH' if highneg else 'MEDIUM' if net<-1 else 'LOW';cat='POSITIVE' if net>1 else 'NEGATIVE' if net<-1 else 'NEUTRAL'
    ranked=sorted(es,key=lambda e:abs(event_value(e)),reverse=True)[:4]
    return status,{'base_status':base,'event_risk':risk,'catalyst':cat,'event_score':round(net,2),'events':ranked}
def classify(c,s,e):
    p=c['current_price'];ok=bool(e.get('ok') and e.get('e20') and e.get('e50') and e.get('rsi') is not None)
    ...
    return'WAIT'
def action_score(c,s,e,ev,status):
    p=c['current_price'];trend=0
    if e.get('e20') and e.get('e50') and e.get('e200'):
        trend=100 if e['e20']>e['e50']>e['e200'] else 75 if p>e['e50'] else 45
    pull=90 if status in ('READY','NEAR ENTRY','EVENT HOLD') else 65 if status=='WAIT' else 30 if status=='OVEREXTENDED' else 20
    rr=75 if status in ('READY','NEAR ENTRY','EVENT HOLD') and e.get('e50') else 50
    rs=max(0,min(100,50+s['rs']*2));event=max(0,min(100,50+ev['event_score']*8));der=70 if e.get('deriv') else 40
    val=.4*((trend+pull)/2)+.25*rs+.15*rr+.15*event+.05*der
    if status in ('INVALIDATED','UNVERIFIED'):val=min(val,35)
    if status=='OVEREXTENDED':val=min(val,55)
    if status=='EVENT HOLD':val=min(val,65)
    return round(val)
def row(c,s,e=None,tier='OPPORTUNITY',events=None,market_risk='LOW'):
    ...
    st=classify(c,s,e);st,ev=event_overlay(st,c['symbol'].upper(),events or [],market_risk);conf=min(100,45+(35 if e.get('ok') else 0)+(15 if e.get('deriv') else 0)+(5 if e.get('oi24') is not None else 0));act=action_score(c,s,e,ev,st)
    return {'id':c['id'],'symbol':c['symbol'].upper(),'name':c['name'],'rank':c['market_cap_rank'],'price':c['current_price'],'tier':tier,'quality':s['score'],'final':act,'action_score':act,'confidence':conf,'status':st,'base_status':ev['base_status'],'event_risk':ev['event_risk'],'catalyst':ev['catalyst'],'event_score':ev['event_score'],'events':ev['events'],'data_source':e.get('source'),'d7':round(s['d7'],2),'d30':round(s['d30'],2),'rs':round(s['rs'],2),'funding':e.get('funding'),'oi24':e.get('oi24'),'e20':e.get('e20'),'e50':e.get('e50'),'rsi4h':e.get('rsi')}
def regime(coins):
...
def update_performance(rows):
    perf=load_json(PERF_PATH,{'version':'1.0','signals':[]});now=datetime.now(timezone.utc).isoformat();sig=perf.setdefault('signals',[]);open_keys={(x.get('symbol'),x.get('opened_status')) for x in sig if not x.get('closed')}
    for r in rows:
        if r['status'] in ('READY','NEAR ENTRY') and (r['symbol'],r['status']) not in open_keys:
            sig.append({'symbol':r['symbol'],'opened_at':now,'opened_status':r['status'],'entry_price':r['price'],'action_score':r['action_score'],'event_score':r['event_score'],'rank':r['rank'],'closed':False})
        for x in sig:
            if x.get('symbol')==r['symbol'] and not x.get('closed'):
                ret=(r['price']/x['entry_price']-1)*100 if x.get('entry_price') else None;x['last_price']=r['price'];x['last_return_pct']=round(ret,2) if ret is not None else None;x['mfe_pct']=round(max(x.get('mfe_pct',-999),ret),2) if ret is not None else x.get('mfe_pct');x['mae_pct']=round(min(x.get('mae_pct',999),ret),2) if ret is not None else x.get('mae_pct');x['last_status']=r['status'];x['last_checked_at']=now
                if r['status'] in ('INVALIDATED','OVEREXTENDED'):
                    x['closed']=True;x['closed_at']=now;x['close_reason']=r['status']
    perf['updated_at']=now;PERF_PATH.write_text(json.dumps(perf,ensure_ascii=False,indent=2))
...
def main():
    ...
    cache=load_json(CACHE_PATH,{});market_risk,events=active_events();btc=next(x for x in coins if x['id']=='bitcoin');inv=[c for c in coins if not excluded(c)];core=inv[:25];opp=inv[25:100];disc=inv[100:300];scored={c['id']:score(c,btc) for c in inv};core_rows=[row(c,scored[c['id']],enrich(c,cache),'CORE',events,market_risk) for c in core];core_rows.sort(key=lambda x:x['rank'])
    opp_all=[row(c,scored[c['id']],None,'OPPORTUNITY') for c in opp];opp_all.sort(key=lambda x:(x['quality'],x['rs']),reverse=True);promotions=[x for x in opp_all if x['quality']>=62 and x['d30']>0 and x['rs']>2][:10]
    discovery=sorted([row(c,scored[c['id']],None,'DISCOVERY') for c in disc if scored[c['id']]['score']>=64 and scored[c['id']]['d30']>0 and scored[c['id']]['rs']>5],key=lambda x:x['quality'],reverse=True)[:10]
    sp=DATA/'monitor_state.json';prev=load_json(sp,{}).get('states',{});states={r['id']:r['status'] for r in core_rows};alerts=[{'id':r['id'],'symbol':r['symbol'],'from':prev.get(r['id']),'to':r['status'],'price':r['price'],'final':r['final']} for r in core_rows if meaningful(prev.get(r['id']),r['status'])];update_performance(core_rows);verified=sum(r['status']!='UNVERIFIED' for r in core_rows);cached=sum('cache' in (r.get('data_source') or '') for r in core_rows)
    result={'version':'4.0','updated_at':datetime.now(timezone.utc).isoformat(),'market_risk':market_risk,'active_event_count':len(events),'regime':regime(coins),'tiers':{'core_count':len(core),'opportunity_count':len(opp),'discovery_count':len(disc)},'data_quality':{'core_verified':verified,'core_unverified':len(core)-verified,'derivatives_verified':sum(r.get('funding') is not None or r.get('oi24') is not None for r in core_rows),'ready_count':sum(r['status']=='READY' for r in core_rows),'cache_used':cached},'core':core_rows,'opportunity':opp_all[:15],'promotions':promotions,'discovery':discovery,'alerts':alerts,'performance_tracking':{'enabled':True,'file':'performance_history.json'}}
