...
def action(c,s,e,ev,st):
 trend=100 if e.get('e20') and e.get('e50') and e.get('e200') and e['e20']>e['e50']>e['e200'] else 70;pull=95 if st=='CONFIRMED' else 85 if st in('NEAR ENTRY','EVENT HOLD') else 55;rs=max(0,min(100,50+s['rs']*2));event=max(0,min(100,50+ev['event_score']*8));v=.4*((trend+pull)/2)+.25*rs+.15*75+.15*event+.05*(70 if e.get('deriv') else40)
